...
policies_static_goal = np.zeros([gw.mdp.horizon, len(gw.mdp.states)])

for time_index in range(gw.mdp.horizon):
    gw.mdp.update_reward(targets_path[0][time_index])
    ...
    for state_index in range(len(gw.mdp.states)):
        ...
        policies_static_goal[time_index, state_index] = temp[0, state_index]
np.save('static_policies', policies_static_goal)
